...
- `--out-dir` - директория для сохранения моделей и состояний (default: текущая директория)
...
Используйте `--out-dir` для раздельного хранения моделей разных экспериментов:
...
.
├── experiment1/
│   ├── models/     # Модели эксперимента 1
│   └── states/     # Состояния эксперимента 1
├── experiment2/
│   ├── models/     # Модели эксперимента 2
│   └── states/     # Состояния эксперимента 2
├── experiment3/
│   ├── models/     # Модели эксперимента 3
│   └── states/     # Состояния эксперимента 3
└── runs/           # Общие TensorBoard логи для всех экспериментов
```

### Сравнение гиперпараметров

TensorBoard автоматически записывает все гиперпараметры и финальные метрики. Все запуски сохраняются в общую директорию `./runs/` с уникальными метками времени и доступны для сравнения во вкладке **HPARAMS** в TensorBoard.
